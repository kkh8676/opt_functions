    ...
    def compute_acquisition_function_CE(self, acquisition_function, grid, tasks, fast_update):
        ...
        while(True):
            ...
            for item in elite_set:
                cov_mat = cov_mat + np.matrix(np.matmul(np.reshape(item,(self.num_dims,1)),np.reshape(item,(self.num_dims,1)).T ))
            ...
            stopping_criteria = (sum_T >= total_Simul_Budget)
