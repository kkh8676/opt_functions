...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%% newly suggesting algorithm %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

# Algorithm PESC_with_KN

# Input: a black box function with unknown mean f
# 	for n = 1,2,...,N do 
# 		candidate X, current data Dn
#		Sample Mk = n0 number of hyperparameter values {ϕ^k}
#		
# 		for k=1,2,...,n0 do
# 			for j =1,2,...,Mj do
# 				Sample f^kj ~ p(f|Dn, φ^j, ϕ^k)
# 				set x_star^kj = argmin f^kj(x)
# 				perform EP and get sample of acquisition function values A_kj = 0.5log(σ_k^2(x))-0.5log(σ_k^2(x|x_star_^kj))
# 				Abar = { ((k-1)*Mj + (j-1))Abar + A } / ((k-1)Mj + j)
#
# 			end for 
# 		end for
#
...
# 		for j = 1,2,...,Mj do
# 			Sample f^new,j ~ p(f|Dn, φ^j, ϕ^new)
# 			set x_star^new,j = argmin f^newj(x)
# 			perform EP and get a ample of acqusition function value A_newj = 0.5log(σ_new^2(x))-0.5log(σ_new^2(x|x_star^newj))
# 			Abar = { Abar * (Mk * Mj + (j-1)) + A_newj } / (Mk * Mj + j)
#
# 	Query the black box function at Xn = I to obtain yn
# 	end for
#
# return XN^tilda = argmax μ_N(x)
...
# nlopt is used in optimization acquisition function and optimizing sample path
# optimizing poesterior mean μ(x)
try:
    import nlopt
except:
...
class DefaultChooser(object):
    ...
    def compute_acquisition_function_KN(self, acquisition_function, grid, tasks, fast_update):
        ...
        while(True):
            ...
            for i in I2:
                for l in I2:
                    W = max(0, (delta2 / (2*r2)) * (((h_square2 * sample_var_list2[i][l])/ (delta2**2))-r2))
        ...
        best_acq_value = sample_avg_acq2[best_acq_ind2]
