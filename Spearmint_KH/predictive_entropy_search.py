...
# 3. logSumExp_scalar
# 4. logSumExp
# 5. log_1_plus_exp_x_scalr
# 6. log_1_plus_exp_x
# 7. log_1_minus_exp_x_scalar
# 8. log_1_minus_exp_x
# 9. chol2inv
# 10. matrixInverse
# 11. ep
# 12. updateEPsolution
# 13. checkConstraintPSD
# 14. updateMarginals
# 15. updateFactors
# 16. gp_prediction_given_chol_K
# 17. predictEP
# 18. sample_gp_with_random_features
# 19. global_optimization_of_GP_approximation
# 20. evaluate_acquisition_funcntion_given_EP_solution

# PES instance
# 1. create_acquisition_function : creating acquisition function and returns acquisition function
# 2. performEPandXstarSamplingForOneState
...
def ep(obj_model, con_models, x_star, minimize=True):
    ...
    """ X contains X_star """
    # X contains observed input and sampled x_star
    ...
    for t in all_tasks:
        ...
        # Why using cholesky decomposition??
    ...
    # what is the meaning of the jitter??
    # For stability
    jitter = dict()
    ...
    a = {
        'obj'      : obj,
        'constraints': con,
        'Ahfhat'   : np.zeros((n, 2, 2)), # intiialize approximate factors to 0, # A_n
        'bhfhat'   : np.zeros((n, 2)),    # b_n
        'ahchat'   : defaultdict(lambda: np.zeros(n)), #d_n^k
        'bhchat'   : defaultdict(lambda: np.zeros(n)), #e_n^k
        'agchat'   : defaultdict(lambda: np.zeros(1)), #g_k
        'bgchat'   : defaultdict(lambda: np.zeros(1)), #h_k
        'm'        : defaultdict(lambda: np.zeros(n+1)),  # marginals, #m_i
        'V'        : defaultdict(lambda: np.zeros((n+1, n+1))), #V_i
        'cholV'    : dict(),
        'mc'       : dict(),
        'Vc'       : dict(),
        'cholVc'   : dict(),
    ...
    damping     = 1.0
    iteration   = 1
    # not convergence and n>0 is the condition
    # convergence boolean variable will be changed if the convergence condition is satisfied......
    # so when convergence condition is satisfied, not getting in the while loop
...
# Need to do this overall states. Ok...
# so we pass in a single EP solution, and the corresponding x_star
# and we assume the state of the models matches these...
# Using this method in PESC-F
# updating EP solution of previous iteration instead computing whole EP solution
...
def updateMarginals(a): # stuff below A.9
    ...
    # for the objective
    vTilde = np.zeros((n+1,n+1))
    # : means search space X all the entries......
    vTilde[np.eye(n+1).astype('bool')] = np.append(a['Ahfhat'][:, 0, 0], np.sum(a['Ahfhat'][: , 1, 1]))
    vTilde[:n, -1] = a['Ahfhat'][:, 0, 1]
    vTilde[-1, :n] = a['Ahfhat'][:, 0, 1]
    # if eigenvalue of a['VpredInv'][obj] + vTilde is less than zero, it means that matrix is not positive semi definite.
    if np.any(npla.eigvalsh(a['VpredInv'][obj] + vTilde) < 1e-6):
        raise npla.linalg.LinAlgError("Covariance matrix is not PSD!")    

    # PESC equation 33,34 part i think,,,,,
    a['V'][obj] = matrixInverse(a['VpredInv'][obj] + vTilde)
    ...
    for c in constraints:
        # S_k+1 part ........
        # what is ahchat and agchat? d_n and g_n
        vTilde = np.diag(np.append(a['ahchat'][c], a['agchat'][c]))
        if np.any(npla.eigvalsh(a['VpredInv'][c] + vTilde) < 1e-6):
            raise npla.linalg.LinAlgError("Covariance matrix is not PSD!")    

        a['V'][c] = matrixInverse(a['VpredInv'][c] + vTilde) # V_i 
        mTilde = np.append(a['bhchat'][c], np.sum(a['bgchat'][c])) # bhchat and bgchat is e_n and h_n
        a['m'][c] = np.dot(a['V'][c], np.dot(a['VpredInv'][c], a['mPred'][c]) + mTilde)
...
def updateFactors(a, damping, minimize=True):
    ...
    k = len(constraints)
    n = a['n'] # number of locations at which objective function is evaluated

    for i in xrange(n):
        # i the element x in search space X
        ...
        mfOld = spla.cho_solve((cholVfOldinv,False), spla.cho_solve((cholVf,False), a['m'][obj][[i, n]]) - a['bhfhat'][i,:]) # A.15 # Equation 38 b^-n part

        vcOld = np.zeros(k)
        mcOld = np.zeros(k)
        # enumerate?
        # j constraint number in constraint only index
        # c constraint number in objective and constraint included index
        ...
        logZ        = logSumExp(logProdPhis + logcdf_robust(alpha),  log_1_minus_exp_x(logProdPhis)) # log of A.18, #Equation40 to logarithm
...
class PES(AbstractAcquisitionFunction):
    ...
    def create_acquisition_function(self, obj_model_dict, con_models_dict,
        ...
        # Do the actual EP or x* sampling, for all states (for all hyperparameter state)
        # Saving information in this PES instance ### self.cached_x_stars ### self.cached_EP_solutions
        # Maybe its data type is 'list' 
        # assigning model state in function_over_hypers function
        # that state is saved in gp model instance of each objective and constraints 
        # and then perform EP and sample Xstars.......                                   
        ...
        # cand? : where is it come from?
        # create_acquisition_function function returns this "acquisition" as function 
        # returns the function!!
        # cand comes "grid" in default_chooser.py 
        def acquisition(cand, compute_grad=False, tasks=None):
            ...
            # hasing the grid candidate string
            ...
            # acq_dict 
            # key : cache_key = (inputs_hash, obj_model.state)
            # 
            ...
            for i in xrange(num_x_star_samples):
                ...
                # evaluate_acquisition_function_given_EP_solution function returns "acq" dictionary
                # t is the key element of "acq" dictionary
                # val is the value of that key element of "acq" dictionary
    ...
    # Getting sampled X star and EP solution of each hyperparameter state
    # saving information in self.cached_x_star[obj_model.state]
    #                       self.cached_EP_solutions[obj_model.state]
    def performEPandXstarSamplingForOneState(self, obj_model, con_models_dict, fast, 
        ...
        for i in xrange(num_x_star_samples):
            ...
            if fast:
                # using PESC-F 
                # there is data in epSolution and x_stars
                ...
                # sample x*
                    ...
                    # Using optimization algorithm, getting sampled x_star from sample path f
