...
class DefaultChooser(object):
    ...
    def fit(self, tasks, hypers=None, fit_hypers=True):
        ...
        # Take One another hyperparameter and update the state size of the GP model
    # We can refre the total number of state via "self.num_states"
    def fit_another(self, inputs, values, pending=None, hypers=None, reburn=False, fit_hypers=True):
        # Set the data for the GP
        self._inputs = inputs
        self._values = values

        self._fantasy_values_list = [] # fantast of pendings

        # Inirailize the GP with hypers if provided, or else set them to their default
        if hypers:
        ...
        if fit_hypers:
            # self._hypers_list : we already have "_hypers_list" with self.num_states number of element
            # self._cache_list  : we already have "_cache_list"  with self.num_states number of element, cholesky

            # chain_length has been already burned, if we are using fit_another function
            #self.chain_length = 0   # chain of hypers

            # Burn samples (if needed)
            # Burning samples process in not needed, because is has been burned already in "fit" function
            ...
            # Now collect another samples (sets self._hypers_list)
            # Don't use original "_collect_samples" function
            # just append self._hypers_list 
            # add another state 
            ...
            self._hypers_list.append(self.to_dict()['hypers'])
            self.chain_length += 1

            # Now we have another states
            self.num_states += 1
        
        # self._cache_list is already fiiled with num_states number 
        # self._cahce_list = []
    ...
    def compute_acquisition_function_CE(self, acquisition_function, grid, tasks, fast_update):
        ...
        acq_candidates = avg_hypers(self.model.values(), acquisition_function, 
        ...
        elite_set = []
        for i in range(m):
            origin_index = acq_candidates.index(copied[i])
            elite_set.append(acq_candidates[origin_index])


        mu = np.average(elite_set, axis=0)
        cov = np.cov(elite_set)
        v = (mu, cov)
        ...
        while(!stopping_criteria):
            ...
            while(True):
                # one sample from pdf
                x = np.random.multivariate_normal(v[0],v[1])
            ...
            for i in range(N):
                ...
                inputs_hash = hash(candidate.tostring())
                ...
                for i in xrange(self.options['pes_num_x*_samples']):
                    ...
                    for t,val in pes.evaluate_acquisition_function_given_EP_solution(self.objective_model_dict,self.constraint_models_dict, candidates, ep_sols[i], x_stars[i]).iteritems():
                        temp_acq_dict[t] += val

                for t in temp_acq_dict:
                    temp_acq_dict[t] = temp_acq_dict[t] / float(self.options['pes_num_x_samples'])
        ...
        best_acq_value = avg_hypers(self.models.values(), acquisition_function, 
                                                mu, compute_grad = False, tasks = tasks)
