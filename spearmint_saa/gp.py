...
class GP(AbstractModel):
    ...
    def _collect_samples(self, num_samples):
        ...
        for i in xrange(num_samples):
            for sampler in self._samplers:
                # sample method contains 'set_params_from_array' method
                # so 'params' variable in GP instance is modified by that method
                # how transmission param instance to GP instance?
                sampler.sample(self)

            # how can we modify 'params' ??
            # where??????
    ...
    def _collect_samples_incre(self, num_samples):


        for sampler in self._samplers:
            logging.debug('  Increasing Sample %d samples of %s with %s' % (num_samples, ', '.join(['%s(%d)'%(param.name, param.size()) for param in sampler.params]), sampler.__class__.__name__))
        logging.debug('')
    ...
    # 20.08.06
    # for incrementally sampling hyperparameters
    # self.sampler don't need the process for burning, so this method is coded
    ...
    def fit_incre(self, inputs, values, pending=None, hypers=None, reburn=False, fit_hypers=True, increasing_num):
        ...
        if fit_hypers:
            # self._hypers_list = []  # samples hypers
            # self._cache_list  = []  # caching cholesky
            #self.chain_length = 0   # chain of hypers # this process in not needed in here method
            ...
            # Now collect some samples (sets self._hypers_list)
            # this process is   need to be modified....
            # because in _collect_samples method 'GP_instance._hypers_list' is modified
            self._collect_samples_incre(increasing_num)
