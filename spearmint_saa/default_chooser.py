...
class DefaultChooser(object):
    ...
    def fit(self, tasks, hypers=None, fit_hypers=True):
        ...
        # hypers have just one set of hyperparameters
        ...
        for task_name, task in tasks.iteritems():
            ...
            if np.array_equal(self.models[task_name]._inputs, inputs) and \
               ...
               not self.options["always_sample"]:
                ...
                if np.array_equal(self.models[task_name].pending, pending):
                    pass # means this if condition, we do nothing 
                else:
                    logging.debug('Adding pending samples to %s for %s task' % (model_class_name, task_name)) # debug only
                    # why this process is needed?
                    # only pending changes, what the hell for the inputs and values ?
                    # without changing hyperparameters, just changing 'pending' in GP instance
            ...
            else: # if there is change in inputs or values ..........
                if fit_hypers:
                    logging.info('Fitting %s to %d data for %s task...' % (model_class_name, len(values), task_name))
                # fit_hypers is False, deals it in GP model's fit method 
                # if hypers dictionary has no key task_name, get method returns None
            ...
            if self.options['scale_duration']: # for decoupled case....
        ...
        # returns only one sample of hyperparameters
        return new_hypers
    ...
    def suggest(self, task_couplings):
        ...
        while(True):
            ...
            stopping_criteria = gotLen and unchanged_seq
            if(stopping_criteria): 
                break
            
            # Stopping criteria is not satisfied.......
            ...
            for task_name_key, task in self.tasks.iteritems():
                new_hyper[task_name_key] = self.models[task_name_key].fit_incre(
                    ...
                    hypers= None,
                    fit_hypers = True,
                    increasing_num = hyper_increasing_num)


            # getting acquisition function -> performEPandXStarSamplingForOneState method
            # function_over_hypers(models, fun, *fun_args, ** fun_kwargs)
            # in PES.py 
    ...
    # 20.08.19
    # create_acquisition_function part should be modified.....
    # this version is for unconstrained version
    # so if wanna applicate to constrained version, need to modified
    def suggest_v2(self, task_couplings):
        ...
        sample_size_N = 2
        sample_size_N_prime = 5
        replication_M = 5
        tolerance_epsilon = 0.05

        relative_gap_thres = 0.05  # this variable is needed??

        hyper_increasing_num = 5 # increasing Rule of N,N'
    ...
    def suggest_v3(self, task_couplings):
        ...
        relative_gap_thres = 0.5  # this variable is needed??
        ...
        while(True):
            ...
            if(round_num == 0):
                ...
                #logging.info("Before")
                #logging.info(len(self.acquisition_function_instance.cached_x_star))

                logging.info("Calculating %s in round %d"%( self.acquisition_function_name, round_num))
            ...
            #logging.info(self.acquisition_function_instance.cached_x_star)
            #logging.info(self.acquisition_function_instance.cached_EP_solutions)
            # initialize/create the acquisition function
            #logging.info("creating acquisition process is bottleneck")
            ...
            # logging.info("threshold value is %s"%str(np.abs(relative_gap_thres * est_v_star)))
            # logging.info(" ")
            logging.info("optimality gap is %f and threshold value is %f"%(optimality_gap,np.abs(relative_gap_thres * est_v_star)))
            #if(stopping_criteria or self.objective_model_dict.values()[0].num_states >= 250):
            if(stopping_criteria):
                logging.info("totallly %d sample of hyperparameter has been used in this iteration"%self.objective_model_dict.values()[0].num_states)
                # break the while loop
                break
    ...
    # 20.10.01
    # modified to reuse already sampled hyperparameters
    # create_acquisition_function part
    def suggest_v4(self, task_couplings):
        ...
        relative_gap_thres = 0.50  # this variable is needed??
        ...
        while(True):
            ...
            # initialize/create the acquisition function
            logging.info("Initializing %s in round %d" %( self.acquisition_function_name, round_num))
            logging.info("Current hyperparameter states are %d "%self.objective_model_dict.values()[0].num_states)
            acquisition_function = self.acquisition_function_instance.create_acquisition_function_v2(\
                self.objective_model_dict, self.constraint_models_dict,
                first = is_first, fast=fast_update, grid=acq_grid, current_best=current_best_value,
                num_random_features=self.options['pes_num_rand_features'],
                x_star_grid_size=self.options['pes_x*_grid_size'], 
                x_star_tolerance=self.options['pes_opt_x*_tol'],
                num_x_star_samples=self.options['pes_num_x*_samples'])
            ...
            logging.info("optimality gap : %f , threshold : %f"%(optimality_gap, np.abs(relative_gap_thres * est_v_star)))
    ...
    # modified to reuse already sampled hyperparameters
    # create_acquisition_function part
    # Cumulative SAA , no multistart

    def suggest_v5(self, task_couplings):
        ...
        while(True):
            ...
            if(stopping_criteria):
                ...
                final_task_acqs = defaultdict(list)
                for group, task_group in task_groups.iteritems():
                    final_task_acqs[group] = self.compute_acquisition_function_no_opt_algo(acquisition_function, x_hat_cands, task_group, fast_update)
    ...
    # 20.10.11
    # Cumulative SAA, using SAA replication
    # when you use m replication of SAA problem, you have 'm' number of x_canididates
    # previously, you used only x_candidates for final solution
    def suggest_v6(self, task_couplings):
        ...
        while(True):
            ...
            for m in range(replication_M):
                ...
                for group, task_group in task_groups.iteritems():
                    ...
                    task_acqs[group] = self.compute_acquisition_function_for_specific_hyper_v2(acquisition_function, m, sample_size_N, acq_grid, task_group, fast_update)
                    # v2 compute_acquisition_function_for_specific_hyper_v2 
                    # not using deterministic optimization algorithm!
                    # this compute_acquisiton_function should be modified 
                    # because that function perform averaging process for all states of GP instance
                    # in this algorithm, we should average of acquisition functions from newly sampled hyperparameter
            ...
            if(stopping_criteria):
                ...
                final_task_acqs = defaultdict(list)
                for group, task_group in task_groups.iteritems():
                    # using original compute_acquisition_function, so use all the states of current GP model
                    final_task_acqs[group] = self.compute_acquisition_function(acquisition_function, acq_grid , task_group, fast_update)
    ...
    # using x hat cands......
    # perform determinisitc optimization algorithm for len(x_hat_cands) time!
        ...
        if self.options['optimize_acq']:
            ...
            if nlopt_imported:
                ...
                for point in x_hat_cands:
                    ...
                    logging.info("opt algo result is %s and value %.4f"%(str(cur_x_opt), cur_y_opt))
    ...
    def compute_acquisition_function(self, acquisition_function, grid, tasks, fast_update):
        ...
        logging.info("Totally Computing %s on grid for %s." % (self.acquisition_function_name, ', '.join(tasks)))
    ...
    # 20.10.08
    # for last x_cands,,,,,,,
    # No additional deterministic optimization algorithm

    def compute_acquisition_function_no_opt_algo(self, acquisition_function, grid, tasks, fast_update):
    ...
    # compute_acquisition_function_subset has no deterministic optimization algorithm
    def compute_acquisition_function_subset(self, acquisition_function, grid, num_subset, tasks, fast_update):
        ...
        # if we are doing a fast update, just use one of the hyperparameter samples
        # avg_hypers = function_over_hypers if not fast_update else function_over_hypers_single
        avg_hypers = function_over_hypers_subset
    ...
    def compute_acquisition_function_for_newly_sampled_hyper(self, acquisition_function, newly_sampled_num, grid, tasks, fast_update):
        ...
        # if we are doing a fast update, just use one of the hyperparameter samples
        # avg_hypers = function_over_hypers if not fast_update else function_over_hypers_single
        avg_hypers = function_over_hypers_subset_last

        # Compute the acquisition function on the grid
        grid_acq = avg_hypers(self.models.values(), acquisition_function, newly_sampled_num,
                                        grid, compute_grad=False, tasks=tasks)
    ...
    # using round num, getting specific hypers to compute that state's compute acquisition function
    def compute_acquisition_function_for_specific_hyper(self, acquisition_function, round_num, newly_sampled_num, grid, tasks, fast_update):
        ...
        # if we are doing a fast update, just use one of the hyperparameter samples
        # avg_hypers = function_over_hypers if not fast_update else function_over_hypers_single
        avg_hypers = function_over_hypers_specific

        # Compute the acquisition function on the grid
        grid_acq = avg_hypers(self.models.values(), acquisition_function, round_num,  newly_sampled_num,
                                        grid, compute_grad=False, tasks=tasks)
        ...
        if self.options['optimize_acq']:
            ...
            if nlopt_imported:
                ...
                def f(x, put_gradient_here):
                    ...
                    if put_gradient_here.size > 0:
                        a, a_grad = avg_hypers(self.models.values(), acquisition_function, round_num, newly_sampled_num,
                                x, compute_grad=True, tasks=tasks)
                        put_gradient_here[:] = a_grad.flatten()
                    else:
                        a = avg_hypers(self.models.values(), acquisition_function, round_num, newly_sampled_num,
                                x, compute_grad=False, tasks=tasks)
    ...
    # using round num, getting specific hypers to compute that state's compute acquisition function
    # 20.10.11
    # not using deterministic optimization algorithm from compute_acquisition_fuinction_for_specific_hyper
    def compute_acquisition_function_for_specific_hyper_v2(self, acquisition_function, round_num, newly_sampled_num, grid, tasks, fast_update):
