...
class DefaultChooser(object):
    ...
    def suggest(self, task_couplings):
        ...
        for group, task_group in task_groups.iteritems():
            task_acqs[group] = self.compute_acquisition_function_KN(acquisition_function, acq_grid, task_group, fast_update)
    ...
    def compute_acquisition_function_KN(self, acquisition_function, grid, tasks, fast_update):
        ...
        logging.info("First phase KN procedure has been started!!")
        ...
        while(True):
            ...
            for i in xrange(self.options['pes_num_x*_samples']):
                ...
                for t,val in pes.evaluate_acquisition_function_given_EP_solution(self.objective_model_dict,self.constraint_models_dict, grid, ep_sols[i],x_stars[i]).iteritems():
                    temp_acq_dict[t] += val
        ...
        best_acq_ind = I[0]
        best_acq_location = grid[best_acq_ind] # [x1,x2,x3,x4] 
        best_acq_value = sample_avg_acq[best_acq_ind]

        logging.info("First Phase has been done!!")

        ## %%%%%%%%%%%%%%%%%%%%% first phase has been done!! #############################
        ## %%%%%%%%%%%%%%%%%%%%% second phase has been started!! %%%%%%%%%%%%%%%%%%%%%%%%%



        second_grid = self.generate_grid(100)

        # second_grid (unit) to specific domain process %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

        # Have to be coded.......
        mins = []
        maxs = []

        for dim in range(len(best_acq_location)):
            if(best_acq_location[dim] - 0.05 < 0 ):
                mins.append(0)
                maxs.append(0.1)

            elif(best_acq_location[dim] + 0.05 > 1 ):
                mins.append(0.9)
                maxs.append(1)

            else:
                mins.append(best_acq_location[dim] - 0.05)
                maxs.append(best_acq_location[dim] + 0.05)

        for item in second_grid:
            for dim in range(len(item)):
                item[dim] = mins[dim] + item[dim] * (maxs[dim] - mins[dim])
        ...
        # %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        logging.info("Current R value is %d"%r)
        # parameter assigning process
        ...
        I2 = range(len(second_grid))
        eta2 = 0.5 * (((2 * alpha2) / (k2-1))**(-2/( n02 -1 ) - 1) )
        h_square2 = 2 * eta2 * (n02-1)


        # Getting acqusiition function values at second_grid points

        # do not use this avg_hypers function?, there is a lot of acquisition_function stated in model instance
        # Using all those hyperparameters which is sampled in the first phase....
        # There will be 'r' number of hyperparameters in GP instance...

        second_grid_acq = avg_hypers(self.models.values(), acquisition_function,
            second_grid, compute_grad = False, tasks = tasks)


        # For sample averaged grid_acqusition function value
        sample_avg_acq2 = copy.deepcopy(second_grid_acq)
        ...
        for key in sorted(self.acquisition_function_instance.stored_acq.keys()):
            if(key not in cache_key_list):
                logging.info("here!!")
                sampled_acq_dict = self.acquisition_function_instance.stored_acq[key]
                sampled_acq_list2.append(sum([sampled_acq_dict[task] for task in tasks]))

        for i in I2:
            sample_vars = []
            for l in I2:
                sum_of_difference_square = 0
                mean_sample_diff = second_grid_acq[i] - second_grid_acq[l]
                for j in range(n02):
                    sample_diff = sampled_acq_list2[j][i] - sampled_acq_list2[j][l]
                    sum_of_difference_square += (sample_diff - mean_sample_diff)**2

                sample_var = (1/(n02-1)) * sum_of_difference_square
                sample_vars.append(sample_var)
            sampled_var_list2.append(sample_vars)

        
        # Second phase KN procedure
        logging.info("Second phase KN procedure has been started!!")
        while(True):

            # Screening process
            logging.info("I2 length is %d"%len(I2))
            logging.info("sampled_var_list2 length is %d"%len(sampled_var_list2))
            for i in I2:
                for l in I2:
                    W = max(0, (delta2 / (2*r2)) * (((h_square2 * sampled_var_list2[i][l])/ (delta2**2))-r2))
                    screen_cond = (sample_avg_acq2[i] < sample_avg_acq2[l] - W)
                    if(screen_cond):
                        I2.remove(i)
                        break
            ...
            # perform EP and X star sampling in current state
            # perform EP and X star samplingForOneState
            ...
            inputs_hash2 = hash(grid.tostring())

            cache_key = (inputs_hash2, self.objective_model_dict.values()[0].state)

            if cache_key in self.acquisition_function_instance.stored_acq:
                return sum([self.acquisition_function_instance.store_acq[cache_key][task] for task in tasks])

            if len({model.state for model in models}) != 1:
                raise Exception("Models are not all at the same state")

            N_cand = second_grid.shape[0]
            ...
            temp_acq_dict2 = defaultdict(lambda : np.zeros(N_cand))
            ...
            for i in xrange(self.options['pes_num_x*_samples']):
                ...
                for t,val in evaluate_acquisition_function_given_EP_solution(self.objective_model_dict,self.constraint_models_dict, second_grid, ep_sols[i], x_stars[i]).iteritems():
                    temp_acq_dict2[t] += val

            for t in temp_acq_dict2:
                temp_acq_dict2[t] = temp_acq_dict2[t] / float(self.options['pes_num_x*_samples'])

            self.stored_acq[cache_key] = temp_acq_dict2

            if tasks in None:
                tasks = temp_acq_dict2.keys()

            newly_sampled_acq2 = sum([temp_acq_dict2[task] for task in tasks])

            # sample_avg_acq2
            # newly_sampled_acq2

            sample_avg_acq2 = list(map(lambda x,y: (r2*x + y)/(r2+1), sample_avg_acq2, newly_sampled_acq2))

            r2 += 1

        # second KN procedure while loop ended

        best_acq_ind2 = I2[0]
        best_acq_location2 = second_grid[best_acq_ind2]
        best_grid_acq_value2 = sample_avg_acq2[best_acq_ind2]


        # Optimizing process again......
        has_grads = self.acquisition_function_instance.has_gradients
        ...
        if self.options['optimize_acq']:
            ...
            if self.options['check_grad']:
                check_grad(lambda x: avg_hypers(self.models.values(), acquisition_function, 
                    x, compute_grad=True), best_acq_location2)
            ...
            if nlopt_imported:
                ...
                x_opt = opt.optimize(best_acq_location2.copy())
                ...
                if (returncode > 0 or returncode==-4) and y_opt > best_grid_acq_value:
                    print_nlopt_returncode(returncode, logging.debug)
                    best_acq_location2 = x_opt
                    best_acq_value2 = y_opt
                else:
                    best_acq_value2 = best_grid_acq_value2
                ...
                bounds = [(0,1)]*self.num_dims
                x_opt, y_opt, opt_info = spo.fmin_l_bfgs_b(f, best_acq_location2.copy(), 
                    bounds=bounds, disp=0, approx_grad=not has_grads)
                y_opt = -y_opt
                # make sure bounds are respected
                x_opt[x_opt > 1.0] = 1.0
                ...
                if y_opt > best_grid_acq_value2:
                    best_acq_location2 = x_opt
                    best_acq_value2 = y_opt
                else:
                    best_acq_value2 = best_grid_acq_value2

            logging.debug('Best %s before optimization: %f' % (self.acquisition_function_name, best_grid_acq_value2))
            logging.debug('Best %s after  optimization: %f' % (self.acquisition_function_name, best_acq_value2))

        else:
            # do not optimize the acqusition function
            logging.debug('Best %s on grid: %f' % (self.acquisition_function_name, best_grid_acq_value2))        

        logging.info("best acq value is %f"%best_acq_value2)
        return {"location" : best_acq_location2, "value" : best_acq_value2}
