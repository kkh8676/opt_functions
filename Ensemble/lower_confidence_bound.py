...
def compute_lcb(model, pred, ei_target=None, compute_grad=True, nu=0.2):

	if model.pending:
		return compute_lcb_pending(model, pred, ei_target, compute_grad)
	...
	# How can we set the beta constant??
	delta    = 0.1
	sigma    = np.sqrt(var)
	t        = len(model._values)
	dim      = model.num_dims
	...
	g_lcb_m  = 1
	g_lcb_s2 = -0.5 * np.sqrt(nu * beta)/ sigma
...
def constraint_weighted_ei(obj_model, constraint_model, cand, current_best, compute_grad):

	return


def lcb_evaluate_constraint_only(obj_model, constraint_models, cand, current_best, compute_grad):
