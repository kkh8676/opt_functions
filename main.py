    ...
    while True:
        ...
        for resource_name, resource in resources.iteritems():
            ...
            if resource.acceptingJobs(jobs):
                ...
                suggested_input, suggested_tasks, suggested_acq , xstar_samples, uncond_Vars, cond_Vars= chooser.suggest(task_couplings)
                logging.info("in main.py suggested acq is %f"%suggested_acq)
                
                suggested_task = suggested_tasks[0] # hack, deal with later

                x_star_samples_dict = dict()
                uncond_Vars_dict = dict()
                cond_Vars_dict = dict()

                for item in xstar_samples:
                    temp_list = []
                    for i in range(len(xstar_samples[item]))  :
                        temp_x_sample = chooser.input_space.from_unit(xstar_samples[item][i]).flatten()
                        temp_list.append(temp_x_sample)
                    x_star_samples_dict['kernel %d'%item] = temp_list

                    uncond_Vars_dict['kernel %d'%item] = uncond_Vars[item]
                    cond_Vars_dict['kernel %d'%item] = cond_Vars[item]

                logging.info("xstar_samples[item] is %s"%str(xstar_samples[0]))
                suggested_job = {
                    'id'          : len(jobs) + 1,
                    ...
                    'acq_value'   : suggested_acq,
                    'x star samples' : x_star_samples_dict,
                    'uncond_Vars' : uncond_Vars_dict,
                    'cond_Vars' : cond_Vars_dict
        ...
        # Satisfying termination condition 
        # Code flows in this if condition.....
