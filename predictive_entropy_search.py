...
class PES(AbstractAcquisitionFunction):
    ...
    def __init__(self, num_dims, verbose=True, input_space=None):
        ...
        self.cached_conditioned_Variances = None
        self.cached_unconditioned_Variances = None
    ...
    def create_acquisition_function(self, obj_model_dict, con_models_dict,
        ...
        if not fast:
            ...
            self.cached_conditioned_Variances = defaultdict(list)
            self.cached_unconditioned_Variances = defaultdict(list)
        ...
        # Getting x* and procesesing EP here............

        # self.cahced_x_star is filled in this code......
        ...
        def acquisition(cand, compute_grad=False, tasks=None):
            ...
            #logging.info(cand)  ###################################################
            #logging.info("in current candidate, calculating acquisition x_star %s"%str(x_stars))
            ...
            #logging.info(num_x_star_samples)
            
            for i in xrange(num_x_star_samples):

                # num_x_star_samples means large M 
                # Monte Carlo Sampling level
                ...
                returned_acq,returned_uncon_Var,returned_con_Var = evaluate_acquisition_function_given_EP_solution(obj_model_dict, 
                                                con_models_dict, cand, ep_sols[i], x_stars[i])
                cond_Vars.append(returned_con_Var)
                uncond_Vars.append(returned_uncon_Var)
            ...
            self.cached_conditioned_Variances[obj_model.state] = cond_Vars
            self.cached_unconditioned_Variances[obj_model.state] = uncond_Vars
            ...
            logging.info(sum([acq_dict[task] for task in tasks]))
...
def evaluate_acquisition_function_given_EP_solution(obj_model_dict, con_models, cand, epSolution, x_star):
    ...
    #logging.info("current x_star values is %s"%str(x_star))
    ...
    temp_conditioned_Variances = dict()
    temp_unconditioned_Variances = dict()
    # i think we report cand  and x_star here......
    # how can i report the conditioned Variance and unconditioned Variance..........
    # kihun
    #logging.info("current x_star is %s"%str(x_star))
    #logging.info("in evaluate_acquisition_function cand variable is ")
    #logging.info(cand)
    ...
    for t in unconstrainedVariances:
        ...
        temp_unconditioned_Variances[t] = unconstrainedVariances[t]
        temp_conditioned_Variances[t] = constrainedVariances[t]
        # reporting constrainedVariances and unconstrainedVariances here........
    ...
    return acq, temp_unconditioned_Variances, temp_conditioned_Variances
